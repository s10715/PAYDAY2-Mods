function removeinvisiblewalls()
	...
	for _,unit in ipairs(World:find_units_quick("all", 1)) do
		if unit and alive(unit) and unit.name and unit:name() and CollisionData[unit:name():key()] then
			unit:set_visible(false)
			if Network:is_server() then
				World:delete_unit(unit) -- host only
			elseif managers.network and managers.network._session then
				managers.network:session():send_to_peers('remove_unit', unit) -- this crashes if you're host
			end
			if unit and alive(unit) and unit:id() ~= -1 then
