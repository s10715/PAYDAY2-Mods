...
local function unCustody_bots()
	if not Network:is_server() then return end
	local spawn_on_position = managers.player and managers.player._players[1] and alive(managers.player._players[1]) and (managers.player._players[1]):camera():position()
	for id, data in pairs(managers.criminals._characters) do
		if data.data.ai and not alive(data.unit) and spawn_on_position then
			managers.trade:remove_from_trade(data.name)
			managers.groupai:state():spawn_one_teamAI(false, data.name, spawn_on_position)
		end
	end
end
-- inCustody_bots()
...
-- partly host only (you can only change your state as client, and state won't affect bots except incustody or uncustody)
-- state can be one of those value:
--	- game provided state: standard, arrested, bleed_out, fatal, tased, incapacitated, jerry1, jerry2(jerry2 can't use in the map that doesn't have parachute, or it will crash the game), bipod(need weapon have bipod, or it will crash the game), carry
--	- custom state: disconnect, kick, ban, incustody, uncustody, replenish, invulnerable, vulnerable
...
local function change_player_state(state, peer_id)
	...
	elseif state == "incustody" then -- only affect yourself if not host
		if peer_id == managers.network._session:local_peer():id() then
			inCustody_player()
			inCustody_bots()
	...
	elseif state == "uncustody" then -- only affect yourself if not host
		if peer_id == managers.network._session:local_peer():id() then
	...
	elseif state == "replenish" then -- only affect yourself if not host
		if alive(peer:unit()) and (Network:is_server() or peer_id == managers.network._session:local_peer():id()) then
			if peer:unit():base() and peer:unit():base().replenish then peer:unit():base():replenish() end
			if peer:unit():character_damage() and peer:unit():character_damage().replenish then peer:unit():character_damage():replenish() end
			peer:unit():network():send_to_unit({ "spawn_dropin_penalty", false, false, 1, 0, 0, 0 })
		end
	elseif state == "invulnerable" then -- only affect yourself if not host
		if alive(peer:unit()) and (Network:is_server() or peer_id == managers.network._session:local_peer():id()) then
			pcall(function() peer:unit():character_damage():set_mission_damage_blockers("invulnerable", true) end)
		end
	elseif state == "vulnerable" then -- only affect yourself if not host
		if alive(peer:unit()) and (Network:is_server() or peer_id == managers.network._session:local_peer():id()) then
			pcall(function() peer:unit():character_damage():set_mission_damage_blockers("invulnerable", false) end)
		end
	elseif state == "standard" or state == "arrested" or state == "bleed_out" or state == "fatal" or state == "tased" or state == "incapacitated" or state == "jerry1" or state == "jerry2" or state == "bipod" or state == "carry" then
		-- defult states, only affect yourself if not host
		if peer_id == managers.network._session:local_peer():id() then
...
local function change_my_state(state)
	if managers.network and managers.network._session and managers.network._session:local_peer() then
		change_player_state(state, managers.network._session:local_peer():id())
	end
end
local function change_all_player_state(state)
	-- -- if someone is in the custody, this can't get his peer
...
-- change_player_state("incustody", 3)
-- change_player_state("uncustody", 3)-- if you want red teleport to you, let him die first, and then respawn him

-- change_my_state("arrested") -- player get cuffed
-- change_my_state("standard") -- default mask on state
