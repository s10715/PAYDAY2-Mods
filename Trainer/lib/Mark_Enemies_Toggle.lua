...
--[[ example:
	...
	["auc"] = { -- Under the Hammer Heist
		{ interaction = "pex_get_unloaded_card", icon = "equipment_bank_manager_key", show_distance = true, 
			check_function = function(unit)
				if managers.player and managers.player._equipment and managers.player._equipment.specials and managers.player._equipment.specials["auc_rfid_tag"] then
					return false
				end
				return true
			end
		...
		{ interaction = "auc_search_clues_tablet", icon = "equipment_files", show_distance = true, stealth_only = true, force = true, 
			check_function = function(unit)
				...
				if is_show then
					-- there's three tablets with clues, but you can only interact the first two tablets, showing three waypoints is within expectation
					if unit.damage and unit:damage() and unit:damage()._variables and unit:damage()._variables.var_clue and unit:damage()._variables.var_clue ~= 0 then
						return true
					end
				end
