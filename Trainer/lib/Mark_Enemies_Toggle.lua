...
local important_npc_per_heist = {
	...
	["auc"] = { -- Under the Hammer Heist
		"units/pd2_dlc_auc/characters/civ_female_auc_lead_curator_1/civ_female_auc_lead_curator_1",
		"units/pd2_dlc_auc/characters/ene_male_auc_auctioneer_1/ene_male_auc_auctioneer_1",
	},
	["kosugi"] = { -- Shadow Raid
...
function add_unit_color_contour(unit, color)
...
function remove_unit_color_contour(unit)
	unit:base():swap_material_config()
	managers.occlusion:add_occlusion(unit)
end


-- determine different contour for different unit
local function determine_contour(important_npc_only, unit, unit_type)
	if not unit or not alive(unit) or not unit.contour or not unit:contour() then
		return
	end

	local mark_time_upgrade = managers and managers.player and managers.player.upgrade_value and managers.player:upgrade_value("player", "mark_enemy_time_multiplier", 1) or 1
	mark_time_upgrade = 100000 -- set mark_time_upgrade to a huge value, so that contour won't disappear

	if unit_type == 1 then -- civilians
		if is_important_npc(unit) then
			-- highlight important npc in different contour
			unit:contour():add("hostage_trade", mark_important_npc_sync_to_teammates, mark_time_upgrade)
		elseif not important_npc_only then
			unit:contour():add("medic_heal", mark_sync_to_teammates, mark_time_upgrade)
		end
	elseif unit_type == 2 then -- enemies
		if is_important_npc(unit) then
			-- highlight important npc in different contour
			unit:contour():add("hostage_trade", mark_important_npc_sync_to_teammates, mark_time_upgrade)
		elseif is_hostage(unit, true) and not important_npc_only then
			-- highlight hostage in different contour
			unit:contour():add("friendly", mark_sync_to_teammates, mark_time_upgrade)
		elseif not important_npc_only then
			unit:contour():add("mark_enemy", mark_sync_to_teammates, mark_time_upgrade)
		end
	elseif unit_type == 3 and not important_npc_only then -- security cameras
		unit:contour():add("mark_unit", mark_sync_to_teammates, mark_time_upgrade)
	end
end

local function remove_determined_contour(unit, unit_type)
	if not unit or not alive(unit) or not unit.contour or not unit:contour() then
		return
	end

	if unit_type == 1 then -- civilians
		if unit:contour():has_id("hostage_trade") then unit:contour():remove("hostage_trade", mark_important_npc_sync_to_teammates) end
		if unit:contour():has_id("medic_heal") then unit:contour():remove("medic_heal", mark_sync_to_teammates) end
		if unit:contour():has_id("mark_enemy") then unit:contour():remove("mark_enemy", mark_sync_to_teammates) end
	elseif unit_type == 2 then -- enemies
		if unit:contour():has_id("hostage_trade") then unit:contour():remove("hostage_trade", mark_important_npc_sync_to_teammates) end
		if not unit.in_slot or (not unit:in_slot(16) and not unit:in_slot(22)) then -- don't remove joker/bots' contour
			if unit:contour():has_id("friendly") then unit:contour():remove("friendly", mark_sync_to_teammates) end
		end
		if unit:contour():has_id("mark_enemy") then unit:contour():remove("mark_enemy", mark_sync_to_teammates) end
	elseif unit_type == 3 and not important_npc_only then -- security cameras
		if unit:contour():has_id("mark_unit") then unit:contour():remove("mark_unit", mark_sync_to_teammates) end
	end
end

local function remove_all_units_mark()
	for u_key, u_data in pairs(managers.enemy:all_civilians()) do
		remove_determined_contour(u_data.unit, 1)
	end
...
-- mark all civilians, enemies and cameras
local function mark_all_units(important_npc_only)
	remove_all_units_mark() -- prevent mark twice

	for u_key, u_data in pairs(managers.enemy:all_civilians() or {}) do
		determine_contour(important_npc_only, u_data.unit, 1)
	end
...
local function hide_units_mark()
	remove_all_units_mark()

	Hooks:RemovePostHook("Mark_Enemies_register_civilian")
	...
	DelayedCalls:Add("check_special_units", 0, function() end)
end

local function show_units_mark()
	-- mark all npc in stealth, and only mark important npc in loud
	local important_npc_only = not managers.groupai:state():whisper_mode()
	mark_all_units(important_npc_only)

	Hooks:PostHook(EnemyManager, "register_civilian", "Mark_Enemies_register_civilian", function(self, unit)
		determine_contour(important_npc_only, unit, 1)
	end)
	Hooks:PreHook(EnemyManager, "on_civilian_died", "Mark_Enemies_on_civilian_died", function(self, dead_unit, damage_info)
		remove_determined_contour(dead_unit, 1)
	end)
	Hooks:PostHook(EnemyManager, "register_enemy", "Mark_Enemies_register_enemy", function(self, enemy)
		determine_contour(important_npc_only, enemy, 2)
		if important_npc_only ~= not managers.groupai:state():whisper_mode() then
			-- means heist go loud, need to refresh contour
			hide_units_mark()
			show_units_mark()
		end
	end)
...
--[[ example:
	...
	["chas"] = { -- Dragon Heist
		{ interaction = "invisible_interaction_open_axis", idstring = Idstring("units/pd2_dlc_chas/equipment/chas_interactable_alarmbox/chas_interactable_alarmbox"), icon = "pd2_wirecutter", show_distance = true, stealth_only = true, force = true, 
			check_function = function(unit)
				if unit:damage() and unit:damage()._state.object and unit:damage()._state.object["61e2ed7cb45359d5"] and unit:damage()._state.object["61e2ed7cb45359d5"].set_visibility and unit:damage()._state.object["61e2ed7cb45359d5"].set_visibility[2] then
					return true
				end
			end
		},
	...
	["mad"] = { -- Boiling Point
		{ idstring = Idstring("units/pd2_dlc_mad/props/mad_prop_military_radar/mad_prop_miltary_radar"), icon = "pd2_c4", show_distance = true, check_collision = { position_offset = Vector3(0, 0, 150), }, with_units_exist = { { interaction = "hold_open_bomb_hatch", force = true, }, }, },
		{ idstring = Idstring("units/pd2_dlc_mad/props/mad_prop_military_sam/mad_prop_miltary_sam"), icon = "pd2_computer", show_distance = true, check_collision = { position_offset = Vector3(0, 0, 150), }, with_units_exist = { { interaction = "hold_open_bomb_hatch", force = true, }, }, },
		...
		{ idstring = Idstring("units/pd2_dlc_mad/props/mad_prop_dead_scientist/mad_prop_dead_scientist"), icon = "equipment_hand", show_distance = true, force = true, check_collision = {}, with_units_exist = { { interaction = "hold_start_scan_hand", force = false, }, }, },
	...
	["framing_frame_1"] = { -- Framing Frame, day 1
		{ interaction = "hold_take_painting", icon = "wp_bag", show_distance = true, },
		{ interaction = "open_from_inside", icon = "wp_door", show_distance = true, },
	},
	["framing_frame_3"] = { -- Framing Frame, day 3
		...
		{ interaction = "pickup_keycard", icon = "equipment_bank_manager_key", show_distance = true, },
		{ interaction = "security_station_keyboard", icon = "pd2_computer", show_distance = true, loud_only = true, force = true, check_collision = {}, with_units_around = { { interaction="open_from_inside", find_distance = 500, force = false, }, }, },
	},
	...
	["welcome_to_the_jungle_2"] = { -- Big Oil, day 2
		...
		{ idstring = Idstring("units/payday2/props/gen_prop_lab_clipboards/gen_prop_lab_clipboard"), icon = "equipment_files", show_distance = true, 
			check_function = function(unit)
				if unit.damage and unit:damage() and unit:damage()._state and unit:damage()._state.object and unit:damage()._state.object["b5cd43441c2f6d7e"] and unit:damage()._state.object["b5cd43441c2f6d7e"].set_visibility and unit:damage()._state.object["b5cd43441c2f6d7e"].set_visibility[2] then
					return true
				end
				return false
			end
		},
		{ idstring = Idstring("units/payday2/props/gen_prop_lab_notebooks/gen_prop_lab_notebooks"), icon = "equipment_files", show_distance = true, 
			check_function = function(unit)
				if unit.damage and unit:damage() and unit:damage()._state and unit:damage()._state.object and unit:damage()._state.object["b5cd43441c2f6d7e"] and unit:damage()._state.object["b5cd43441c2f6d7e"].set_visibility and unit:damage()._state.object["b5cd43441c2f6d7e"].set_visibility[2] then
					return true
				end
	...
	["fish"] = { -- The Yacht Heist
		{ idstring = Idstring("units/pd2_dlc_fish/lxy_prop_auction_tag/lxy_prop_auction_tag"), icon = "equipment_rfid_tag_02", show_distance = true, stealth_only = true, with_units_around = { { interaction = "hold_type_in_password", find_distance = 50, force = true, }, }, },
	},
	["spa"] = { -- Brooklyn 10-10
		{ interaction = "gen_pku_crowbar", icon = "equipment_crowbar", show_distance = true, },
	...
	["nightclub"] = { -- Nightclub
		{ interaction = "pickup_keycard", icon = "equipment_bank_manager_key", show_distance = true, },
		{ interaction = "open_from_inside", icon = "pd2_door", show_distance = true, },
		{ interaction = "pick_lock_hard", icon = "pd2_drill", show_distance = true, force = true, 
			check_function = function(unit)
				local check_doors = {
					{ safe_pos=Vector3(177, -7000, 475.303), doors_pos={ Vector3(1304, -6550, 372.5), Vector3(1573, -7106, -25), }, },
					{ safe_pos=Vector3(3443, -5357, 474), doors_pos={ Vector3(3320, -7623, -26), }, },
					{ safe_pos=Vector3(1993, -7292, -425), doors_pos={ Vector3(), }, },
				}
				for _, data in pairs(check_doors) do
					if unit:position() == data.safe_pos then
						for _, door_pos in pairs(data.doors_pos) do
							local found_units = World:find_units_quick("sphere", door_pos, 10, managers.slot:get_mask("all"))
							for _, found_unit in ipairs(found_units) do
								if found_unit and alive(found_unit) and found_unit.interaction and found_unit:interaction() and found_unit:interaction():active() and found_unit:interaction().tweak_data == "open_from_inside" then
									return true
								end
							end
						end
	...
	["fex"] = { -- Buluc's Mansion
		...
		{ interaction = "fex_hold_search_for_clue", icon = "equipment_files", show_distance = true, force = true, 
			check_function = function(unit)
				-- only show waypoint when door can be interacted or in baseroom
				local is_show = false
				for _, unit in pairs(managers.interaction and managers.interaction._interactive_units or {}) do
					if unit.interaction and unit:interaction() and unit:interaction().tweak_data == "fex_press_bat_symbol" and unit:interaction().active and unit:interaction():active() then
						is_show = true
						break
					end
				end
				-- outter baseroom: -2300<x<2800, 3850<y<5500, z<70, inner baseroom: -1100<x<900, 1200<y<4450, z<-85
				...
				if player_pos and ((player_pos.x > -2300 and player_pos.x < 2800 and player_pos.y > 3850 and player_pos.y < 5500 and player_pos.z < 70) or (player_pos.x > -1100 and player_pos.x < 900 and player_pos.y > 1200 and player_pos.y < 4450 and player_pos.z < -85))then
					is_show = true
				end
				if is_show then
					local clues = { ["fex_mayan_door_clue_001"] = true, ["fex_mayan_door_clue_002"] = true, ["fex_mayan_door_clue_003"] = true, ["fex_mayan_door_clue_004"] = true, }
					if unit:unit_data() and clues[unit:unit_data().instance] then
						return true
					end
				end
				return false
	...
	["des"] = { -- Henry's Rock
		{ interaction = "gen_pku_crowbar", icon = "equipment_crowbar", show_distance = true, },
		{ interaction = "hold_search_documents", icon = "equipment_files", show_distance = true, force = true, 
			check_function = function(unit)
				local is_show = false
				if alive(unit) and unit.interaction and unit:interaction() and unit:interaction().active and unit:interaction():active() then
					is_show = true
				end
				for _, unit in pairs(managers.interaction and managers.interaction._interactive_units or {}) do
					if alive(unit) and unit.interaction and unit:interaction() and unit:interaction().tweak_data == "push_button_des" and unit:interaction().active and unit:interaction():active() then
						is_show = true
						break
					end
				end
				-- 2550<x<5900, -3900<y<-2400
				local player_pos = managers and managers.player and managers.player:player_unit() and alive(managers.player:player_unit()) and managers.player:player_unit():movement():m_head_pos()
				if player_pos and (player_pos.x > 2550 and player_pos.x < 5900 and player_pos.y > -3900 and player_pos.y < -2400) then
				else
					is_show = false
				end
				if is_show then
					if unit.damage and unit:damage() and unit:damage()._variables and unit:damage()._variables.var_note and unit:damage()._variables.var_note ~= 0 then
						return true
					end
				end
				return false
	...
	["nmh"] = { -- No Mercy
		{ idstring = Idstring("units/payday2/equipment/gen_equipment_security_camera/gen_equipment_security_camera"), icon = "wp_target", show_distance = false, stealth_only = true, 
			check_function = function(unit)
				if unit and alive(unit) and unit.is_security_camera and not unit:destroyed() then
					return true
				end
			end
		},
	...
	["auc"] = { -- Under the Hammer Heist
		{ interaction = "auc_get_rfid", icon = "equipment_bank_manager_key", show_distance = true, },
		{ interaction = "auc_rfid_code", icon = "equipment_rfid_tag_02", show_distance = true, },
		{ interaction = "auc_rfid_code", icon = "equipment_rfid_tag_02", show_distance = true, stealth_only = true, force = true, with_units_exist = { { interaction = "cas_button_01", }, }, },
		{ interaction = "auc_send_tablet_message", icon = "pd2_computer", show_distance = true, },
		{ interaction = "auc_search_clues", icon = "equipment_files", show_distance = true, stealth_only = true, force = true, 
			check_function = function(unit)
				local is_show = false
				if alive(unit) and unit.interaction and unit:interaction() and unit:interaction().active and unit:interaction():active() then
					is_show = true
				...
				for _, unit in pairs(managers.interaction and managers.interaction._interactive_units or {}) do
					if alive(unit) and unit.interaction and unit:interaction() and unit:interaction().tweak_data == "hold_cut_cable" and unit:interaction().active and unit:interaction():active() then
						is_show = true
						break
					end
				end
				if is_show then
					-- there're 2 books, 2 desk-tablets and 2 standing-tablets might have clues, and you can interact 1 unit for each type, showing 5-6 waypoints is within expectation
					if unit.damage and unit:damage() and unit:damage()._variables and unit:damage()._variables.var_clue and unit:damage()._variables.var_clue ~= 0 then
						return true
					end
				end
			end
		},
		{ interaction = "auc_search_clues_tablet", icon = "equipment_files", show_distance = true, stealth_only = true, force = true, 
			check_function = function(unit)
				local is_show = false
				if alive(unit) and unit.interaction and unit:interaction() and unit:interaction().active and unit:interaction():active() then
					is_show = true
		...
		{ interaction = "auc_pku_statue_special", icon = "wp_bag", show_distance = true, stealth_only = true, force = true, 
			check_function = function(unit)
				if Network:is_client() then return false end
				-- find the right statue, host only
				local is_correct_statue = false
				if unit and alive(unit) and unit.interaction and unit:interaction() and unit:interaction():interact_position() and managers and managers.mission and managers.mission:script("default") and managers.mission:script("default")._elements then
					local statue_locations = {
						["105243"] = { Vector3(-3625, 1825, 130), Vector3(-1960, 3675, 130), Vector3(-1000, 475, 130), },
						["105244"] = { Vector3(-3925, 1825, 130), Vector3(-1960, 3375, 130), Vector3(-1000, 175, 130), },
						["105245"] = { Vector3(-3625, 1625, 130), Vector3(-1760, 3675, 130), Vector3(-800, 475, 130), },
						["105246"] = { Vector3(-3925, 1625, 130), Vector3(-1760, 3375, 130), Vector3(-800, 175, 130), },
					}
					local statue_id = tostring(managers.mission:script("default")._elements[101633]._values.on_executed[1].id)
					for _, pos in pairs(statue_locations[statue_id] or {}) do
						if mvector3.distance(unit:interaction():interact_position(), pos) < 25 then
							is_correct_statue = true
							break
						end
					end
					if not is_correct_statue then return false end

					-- you still have to find at least one clue before you take the statue to make system trigger the event
					for _, unit in pairs(World:find_units_quick("all",1) or {}) do
						if unit and alive(unit) and unit.interaction and unit:interaction() and (unit:interaction().tweak_data == "auc_search_clues" or unit:interaction().tweak_data == "auc_search_clues_tablet") then
							if unit.damage and unit:damage() and unit:damage()._variables and unit:damage()._variables.var_clue and unit:damage()._variables.var_clue ~= 0 then
		...
		{ interaction = "hold_hack_computer", icon = "pd2_computer", show_distance = true, },
		{ interaction = "hold_hack_comp", icon = "pd2_computer", show_distance = true, },
		{ interaction = "auc_cutter_tool", icon = "equipment_born_tool", show_distance = true, },
		{ interaction = "hold_take_fire_extinguisher", icon = "equipment_fire_extinguisher", show_distance = true, },
		{ interaction = "auc_car_keys", icon = "equipment_chavez_key", show_distance = true, },
	},
	["rat"] = { -- Cook Off
		{ interaction = "hydrogen_chloride", icon = "pd2_methlab", show_distance = true, },
		{ interaction = "caustic_soda", icon = "pd2_methlab", show_distance = true, },
	...
	["arm_for"] = { -- Transport: Train Heist
		{ interaction = "pickup_harddrive", icon = "equipment_harddrive", show_distance = true, },
	...
	["family"] = { -- Diamond Store
		{ interaction = "pickup_keycard", icon = "equipment_bank_manager_key", show_distance = true, },
		{ interaction = "numpad_keycard", icon = "icon_locked", show_distance = true, stealth_only = true, },
	},
	["branchbank"] = { -- Bank Heist
		{ interaction = "pickup_keycard", icon = "equipment_bank_manager_key", show_distance = true, },
	},
	...
	["pent"] = { -- Mountain Master
		...
		{ interaction = "cas_bfd_drill_toolbox", icon = "equipment_crowbar", show_distance = true, force = true, distance_limit = Vector3(0, 0, 4000), 
			check_function = function(unit)
				if Network:is_server() then return end
				-- toolbox for client
				if alive(unit) and unit:damage() and unit:damage()._state and unit:damage()._state.object and unit:damage()._state.object["b5cd43441c2f6d7e"] and unit:damage()._state.object["b5cd43441c2f6d7e"].set_visibility and unit:damage()._state.object["b5cd43441c2f6d7e"].set_visibility[2] then
					return true
				end
			end,
		},
...
local common_big_loot_idstrings ={
	...
	{ interaction = "auc_pku_statue", icon = "wp_bag", show_distance = false, },
	...
	{ interaction = "hold_unlock_car", show_distance = false, },

	invalid_interaction_per_heist = {
		["framing_frame_1"] = { "hold_take_painting", }, -- Framing Frame, day 1
		...
		["gallery"] = { "hold_take_painting" }, -- Art Gallery
...
local function add_waypoint(position, waypoint_name, icon, color, show_distance)
	if position and waypoint_name and managers and managers.hud and managers.hud.add_waypoint then
		...
		--[[
		-- only show waypoint when waypoint is in screen
		if waypoint and waypoint.bitmap and waypoint.arrow then
			waypoint.move_speed = 0
			-- waypoint.arrow:set_alpha(0) -- remove arrow
			local callback = function()
				local waypoint = managers.hud and managers.hud._hud and managers.hud._hud.waypoints[waypoint_name]
				if not waypoint then
					if managers.hud then managers.hud:remove_updator(waypoint_name) end
					return
				end
				local cam = managers.viewport:get_current_camera()
				local panel = managers.hud:panel(PlayerBase.PLAYER_INFO_HUD_PD2)
				if not cam or not panel then return end
				local screen_pos = managers.hud._workspace:world_to_screen(cam, position)
				-- local texture, rect = tweak_data.hud_icons:get_icon_data(icon, { 0, 0, 32, 32 }) -- icon size is 32 by default
				local visible = (screen_pos.x > 32 and screen_pos.y > 32 and screen_pos.z > 0) and not panel:outside(screen_pos.x + 32, screen_pos.y + 32)
				waypoint.bitmap:set_visible(visible)
			end
			managers.hud:add_updator(waypoint_name, callback)
		end
		]]--
	end
end
...
local function remove_waypoint(waypoint_name)
	if waypoint_name and managers and managers.hud and managers.hud.remove_waypoint then
...
-- remove all waypoints that we added by default, if unit_list parameter exists, then only remove units' waypoint in this list
local function remove_determined_waypoints(unit_list)
	if unit_list and type(unit_list) == "table" then
		...
		for _, waypoint_name in pairs(global_markenemies_waypoint_cache or {}) do
			remove_waypoint(waypoint_name)
		end
		global_markenemies_waypoint_cache = {}
	end
end
...
local function show_next_waypoint_list(idx)
	if not global_markenemies_waypoint_list_idx or global_markenemies_waypoint_list_idx > 4 then
		global_markenemies_waypoint_list_idx = 0
	end

	-- if assign an idx, then search from this idx
	if idx then
	...
	for i = global_markenemies_waypoint_list_idx + 1, 4 do
		if determine_waypoint(i) then
			global_markenemies_waypoint_list_idx = i
			return
		end
	end

	-- if no waypoint can be showed from next_idx to max_idx, then search from start_idx to current_idx
	for i = 1, global_markenemies_waypoint_list_idx do
		if determine_waypoint(i) then
			global_markenemies_waypoint_list_idx = i
			return
		end
