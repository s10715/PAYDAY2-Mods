...
function PlayerDamage:_check_bleed_out(...)
	if global_autoheal_toggle and self:get_real_health() == 0 then 
		if managers.player and managers.player:has_category_upgrade("player", "damage_health_ratio_multiplier") then
			--self:change_health(self:_max_health() * self._healing_reduction / 100) -- set health to 1% if you have berserker skill
			trigger_berserker()
			self:_send_set_health()
			-- self:_on_damage_event() -- start regenerate armor process
			self:set_armor(self:_max_armor()) -- regenerate armor instantly
			self:_send_set_armor()
		else
			self:band_aid_health() -- fully health yourself
			-- self._unit:base():replenish() -- fully health, reset down counter and refill bullet
		end

		if tweak_data.upgrades and tweak_data.upgrades.first_aid_kit and tweak_data.upgrades.first_aid_kit.revived_damage_reduction then
			-- get 80% damage reduction like reviving by teammate with aced painkiller skill
			local revive_damage_reduction_level = 2
			local tweak = tweak_data.upgrades.first_aid_kit.revived_damage_reduction[revive_damage_reduction_level]
