...
local function set_build(build_string, check_only)
	...
	local function check_dlc(dlc_name)
		if dlc_name and not managers.dlc:is_dlc_unlocked(dlc_name) then
			check_result.missing_dlcs[dlc_name] = true
			return false
		end
		return true
	end
		...
		elseif starts_with(line, "perk_deck=") then -- Perk deck
			...
			if check_only then
				if perk_deck and tweak_data.skilltree.specializations[perk_deck] then
					...
					check_result.perk_deck.missing_dlc = not check_dlc(tweak_data.skilltree.specializations[perk_deck].dlc)
		...
		elseif starts_with(line, "projectile=") then -- Projectile
			...
			if check_only then
				if projectile_name and tweak_data.blackmarket.projectiles[projectile_name] then
					...
					check_result.projectile.missing_dlc = not check_dlc(tweak_data.blackmarket.projectiles[projectile_name].dlc)
			...
			if check_only then
				if weapon_name and tweak_data.weapon[weapon_name] then
					if weapon_category == "primaries" then
						...
						check_result.primary_weapon.missing_dlc = not check_dlc(tweak_data.weapon[weapon_name].global_value) or check_result.primary_weapon.missing_dlc
					...
					elseif weapon_category == "secondaries" then
						...
						check_result.secondary_weapon.missing_dlc = not check_dlc(tweak_data.weapon[weapon_name].global_value) or check_result.secondary_weapon.missing_dlc
					...
					for _, weapon_attachment in pairs(weapon_attachments) do
						if weapon_attachment and tweak_data.weapon.factory.parts[weapon_attachment] then
							if weapon_category == "primaries" then
								check_result.primary_weapon.missing_dlc = not check_dlc(tweak_data.weapon.factory.parts[weapon_attachment] and tweak_data.weapon.factory.parts[weapon_attachment].dlc) or check_result.primary_weapon.missing_dlc
							elseif weapon_category == "secondaries" then
								check_result.secondary_weapon.missing_dlc = not check_dlc(tweak_data.weapon.factory.parts[weapon_attachment] and tweak_data.weapon.factory.parts[weapon_attachment].dlc) or check_result.secondary_weapon.missing_dlc
							end
						end
					end
		...
		elseif starts_with(line, "melee_weapon=") then -- Melee Weapon
			...
			if check_only then
				if melee_weapon_name and tweak_data.blackmarket.melee_weapons[melee_weapon_name] then
					...
					check_result.melee_weapon.missing_dlc = not check_dlc(tweak_data.blackmarket.melee_weapons[melee_weapon_name].dlc)
		...
		elseif starts_with(line, "mask=") then -- Mask
			...
			if check_only then
				if mask_name and tweak_data.blackmarket.masks[mask_name] then
					...
					check_result.mask.missing_dlc = not check_dlc(tweak_data.blackmarket.masks[mask_name].dlc)
