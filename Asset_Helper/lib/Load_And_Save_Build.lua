...
local function set_build(build_string, check_only)
	...
	local check_result = {
		...
		checked = false, -- if anything decoded, false means this is a empty build string
	}
	local function get_localized_dlc_name(dlc_name)
		-- the game didn't provide any function to get localized dlc name, this method is unstable, some dlc name might can't get
		if tweak_data.lootdrop and tweak_data.lootdrop.global_values and tweak_data.lootdrop.global_values[dlc_name] then
			local localized_name = nil
			local name_id = tweak_data.lootdrop.global_values[dlc_name].name_id
			local unlock_id = tweak_data.lootdrop.global_values[dlc_name].unlock_id
			if name_id and managers.localization:exists(name_id) then
				localized_name = tostring(managers.localization:text(name_id))
			end
			if (not localized_name or (localized_name:gsub("^%s*(.-)%s*$", "%1")) == "") and unlock_id and managers.localization:exists(unlock_id) then
				-- lock by community/twitch drops/achievement/...
				localized_name = tostring(managers.localization:text(unlock_id))
			end

			if localized_name and (localized_name:gsub("^%s*(.-)%s*$", "%1")) ~= "" then
				return localized_name
			end
		end
	end
	local function check_dlc(dlc_name, item_name_id)
		if dlc_name and not managers.dlc:is_dlc_unlocked(dlc_name) then
			-- item_name_id can be nil because some internal attachments don't have localized name
			local localized_item_name = item_name_id and managers.localization:text(item_name_id) or nil
			local localized_dlc_name = get_localized_dlc_name(dlc_name)
			table.insert(check_result.missing_dlcs, { item = localized_item_name, dlc = localized_dlc_name })
			return false
		end
		return true
	end
	...
	for _, line in pairs(build_string_list) do
		...
		if starts_with(line, "profile_name=") then -- Profile Name
			...
			if check_only then
				if profile_name and profile_name ~= "" then
					check_result.profile_name = profile_name
				end
			elseif profile_name and profile_name ~= "" then
		...
		elseif starts_with(line, "skill_switch_name=") then -- Skill Switch Name
			...
			check_result.checked = true
			if check_only then
				if skill_switch_name and skill_switch_name ~= "" then
					check_result.skill_switch_name = skill_switch_name
				end
		...
		elseif starts_with(line, "skill_trees=") and not check_result.skill_trees.checked then -- Skill Trees
			...
			if check_only then
				...
				check_result.skill_trees.not_enough_points = spent_points > managers.skilltree:max_points_for_current_level()
		...
		elseif starts_with(line, "perk_deck=") and not check_result.perk_deck.checked then -- Perk deck
			...
			if check_only then
				if perk_deck and tweak_data.skilltree.specializations[perk_deck] then
					...
					check_result.perk_deck.missing_dlc = not check_dlc(tweak_data.skilltree.specializations[perk_deck].dlc, tweak_data.skilltree.specializations[perk_deck].name_id)
				elseif perk_deck then
					check_result.perk_deck.invalid = true
				end
		...
		elseif starts_with(line, "armor=") and not check_result.armor.checked then -- Armor
			...
			if check_only then
				...
				elseif armor_name and armor_name ~= "" then
					check_result.armor.invalid = true
				end
		...
		elseif starts_with(line, "projectile=") and not check_result.projectile.checked then -- Projectile
			...
			if check_only then
				if projectile_name and tweak_data.blackmarket.projectiles[projectile_name] then
					...
					check_result.projectile.missing_dlc = not check_dlc(tweak_data.blackmarket.projectiles[projectile_name].dlc, tweak_data.blackmarket.projectiles[projectile_name].name_id)
				elseif projectile_name and projectile_name ~= "" then
					check_result.projectile.invalid = true
				end
				...
				if not check_result.deployable[slot].checked then
					...
					if check_only then
						...
						if deployable_name and tweak_data.blackmarket.deployables[deployable_name] then
							...
							check_result.deployable[slot].missing_dlc = not check_dlc(tweak_data.blackmarket.deployables[deployable_name].dlc, tweak_data.blackmarket.deployables[deployable_name].name_id)
		...
		elseif (starts_with(line, "primary_weapon=") and not check_result.primary_weapon.checked) or (starts_with(line, "secondary_weapon=") and not check_result.secondary_weapon.checked) then -- Primary Weapon and Secondary Weapon
			...
			if check_only then
				if weapon_name and tweak_data.weapon[weapon_name] then
					if weapon_category == "primaries" then
						check_result.primary_weapon.name = managers.localization:text(tweak_data.weapon[weapon_name].name_id)
						check_result.primary_weapon.missing_dlc = not check_dlc(tweak_data.weapon[weapon_name].global_value, tweak_data.weapon[weapon_name].name_id) or check_result.primary_weapon.missing_dlc
						check_result.primary_weapon.no_empty_slot = not managers.blackmarket:_get_free_weapon_slot(weapon_category)
					elseif weapon_category == "secondaries" then
						check_result.secondary_weapon.name = managers.localization:text(tweak_data.weapon[weapon_name].name_id)
						check_result.secondary_weapon.missing_dlc = not check_dlc(tweak_data.weapon[weapon_name].global_value, tweak_data.weapon[weapon_name].name_id) or check_result.secondary_weapon.missing_dlc
						check_result.secondary_weapon.no_empty_slot = not managers.blackmarket:_get_free_weapon_slot(weapon_category)
					end
					for _, weapon_attachment in pairs(weapon_attachments) do
						if weapon_attachment and tweak_data.weapon.factory.parts[weapon_attachment] then
							if weapon_category == "primaries" then
								check_result.primary_weapon.missing_dlc = not check_dlc(tweak_data.weapon.factory.parts[weapon_attachment].dlc, not tweak_data.weapon.factory.parts[weapon_attachment].inaccessible and tweak_data.weapon.factory.parts[weapon_attachment].name_id) or check_result.primary_weapon.missing_dlc
							elseif weapon_category == "secondaries" then
								check_result.secondary_weapon.missing_dlc = not check_dlc(tweak_data.weapon.factory.parts[weapon_attachment].dlc, not tweak_data.weapon.factory.parts[weapon_attachment].inaccessible and tweak_data.weapon.factory.parts[weapon_attachment].name_id) or check_result.secondary_weapon.missing_dlc
							end
						end
					end
				elseif weapon_name and weapon_name ~= "" then
					if weapon_category == "primaries" then
						check_result.primary_weapon.invalid = true
					elseif weapon_category == "secondaries" then
						check_result.secondary_weapon.invalid = true
					end
				end
			else
		...
		elseif starts_with(line, "melee_weapon=") and not check_result.melee_weapon.checked then -- Melee Weapon
			...
			if check_only then
				if melee_weapon_name and tweak_data.blackmarket.melee_weapons[melee_weapon_name] then
					...
					check_result.melee_weapon.missing_dlc = not check_dlc(tweak_data.blackmarket.melee_weapons[melee_weapon_name].dlc, tweak_data.blackmarket.melee_weapons[melee_weapon_name].name_id)
				elseif melee_weapon_name and melee_weapon_name ~= "" then
					check_result.melee_weapon.invalid = true
				end
		...
		elseif starts_with(line, "mask=") and not check_result.mask.checked then -- Mask
			...
			if check_only then
				if mask_name and tweak_data.blackmarket.masks[mask_name] then
					...
					check_result.mask.missing_dlc = not check_dlc(tweak_data.blackmarket.masks[mask_name].dlc, tweak_data.blackmarket.masks[mask_name].name_id)
				elseif mask_name and mask_name ~= "" then
					check_result.mask.invalid = true
				end
					...
					if not check_result.crew[idx].checked then
						...
						if check_only then
							...
							if crew_weapon_real_name and tweak_data.weapon[crew_weapon_real_name] then
								...
								check_result.crew[idx].missing_dlc = not check_dlc(tweak_data.weapon[crew_weapon_real_name].global_value, tweak_data.weapon[crew_weapon_real_name].name_id) or check_result.crew[idx].missing_dlc
								for _, crew_weapon_attachment in pairs(crew_weapon_attachments) do
									if tweak_data.weapon.factory.parts[crew_weapon_attachment] then
										check_result.crew[idx].missing_dlc = not check_dlc(tweak_data.weapon.factory.parts[crew_weapon_attachment].dlc, not tweak_data.weapon.factory.parts[crew_weapon_attachment].inaccessible and tweak_data.weapon.factory.parts[crew_weapon_attachment].name_id) or check_result.crew[idx].missing_dlc
									end
								end
...
local function check_and_set_build(build_string)
	...
	if not check_result.checked then
		show_diag("Error", "Build not found.")
		return
	end
	...
	if check_result.skill_switch_name then
		local hint = "skill profile name: " .. tostring(check_result.skill_switch_name)
		table.insert(show_msgs, hint)
	end
	if check_result.skill_trees and check_result.skill_trees.spent_points then
		local hint = "skill point: " .. tostring(check_result.skill_trees.spent_points)
		if check_result.skill_trees.not_enough_points then
			hint = hint .. " (not enough points)"
		end
		table.insert(show_msgs, hint)
	end
	if check_result.perk_deck and check_result.perk_deck.name then
		local hint = "perk deck: " .. tostring(check_result.perk_deck.name)
		if check_result.perk_deck.missing_dlc then
			hint = hint .. " (missing dlc)"
		end
		table.insert(show_msgs, hint)
	elseif check_result.perk_deck and check_result.perk_deck.invalid then
		local hint = "perk deck invalid!"
		table.insert(show_msgs, hint)
	end
	if check_result.armor and check_result.armor.name then
		local hint = "armor: " .. tostring(check_result.armor.name)
		table.insert(show_msgs, hint)
	elseif check_result.armor and check_result.armor.invalid then
		local hint = "armor invalid!"
		table.insert(show_msgs, hint)
	end
	if check_result.projectile and check_result.projectile.name then
		local hint = "projectile: " .. tostring(check_result.projectile.name)
		if check_result.projectile.missing_dlc then
			hint = hint .. " (missing dlc)"
		end
		table.insert(show_msgs, hint)
	elseif check_result.projectile and check_result.projectile.invalid then
		local hint = "projectile invalid!"
		table.insert(show_msgs, hint)
	end
	if check_result.deployable then
		for i = 1, 2 do
			if check_result.deployable[i] and check_result.deployable[i].name then
				...
				table.insert(show_msgs, hint)
			elseif check_result.deployable[i] and check_result.deployable[i].invalid then
				local hint = "deployable " .. tostring(i) .. " invalid!"
				table.insert(show_msgs, hint)
			end
		end
	end
	if check_result.primary_weapon and check_result.primary_weapon.name then
		local hint = "primary: " .. tostring(check_result.primary_weapon.name)
		if check_result.primary_weapon.missing_dlc then
			hint = hint .. " (missing dlc)"
		end
		table.insert(show_msgs, hint)
	elseif check_result.primary_weapon and check_result.primary_weapon.invalid then
		local hint = "primary weapon invalid!"
		table.insert(show_msgs, hint)
	end
	if check_result.secondary_weapon and check_result.secondary_weapon.name then
		local hint = "secondary: " .. tostring(check_result.secondary_weapon.name)
		if check_result.secondary_weapon.missing_dlc then
			hint = hint .. " (missing dlc)"
		end
		table.insert(show_msgs, hint)
	elseif check_result.secondary_weapon and check_result.secondary_weapon.invalid then
		local hint = "secondary weapon invalid!"
		table.insert(show_msgs, hint)
	end
	if check_result.melee_weapon and check_result.melee_weapon.name then
		local hint = "melee: " .. tostring(check_result.melee_weapon.name)
		if check_result.melee_weapon.missing_dlc then
			hint = hint .. " (missing dlc)"
		end
		table.insert(show_msgs, hint)
	elseif check_result.melee_weapon and check_result.melee_weapon.invalid then
		local hint = "melee weapon invalid!"
		table.insert(show_msgs, hint)
	end
	if check_result.mask and check_result.mask.name then
		local hint = "mask: " .. tostring(check_result.mask.name)
		if check_result.mask.missing_dlc then
			hint = hint .. " (missing dlc)"
		end
		table.insert(show_msgs, hint)
	elseif check_result.mask and check_result.mask.invalid then
	...
	if next(check_result.missing_dlcs or {}) then
		local hint = "Missing dlc item(s): \n"
		for i = 1, #check_result.missing_dlcs do
			local item = check_result.missing_dlcs[i].item
			local dlc = check_result.missing_dlcs[i].dlc
			if item then
				hint = hint .. "    " .. tostring(item)
				if dlc then
					hint = hint .. " (" .. tostring(dlc) .. ")"
				end
				hint = hint .. "\n"
			end
		end
	...
	local msg_text = ""
	for _, msg in pairs(show_msgs) do
