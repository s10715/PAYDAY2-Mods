...
local function show_engine_waypoint(engine_num, is_send_msg)
	...
	if engine_locations[engine_num] then
		for num, _ in pairs(engine_locations) do
			-- prevent waypoint not disappear when other player take the engine, which won't trigger unit:interaction().interact, also can refresh waypoint when select a different clue
			local wp_name = "engine_waypoint_" .. tostring(num)
	...
	for _, data in pairs(all_engine_clues) do
		table.insert(dialog_data.button_list, {
			...
			callback_func = function()
				for _, num in pairs(data.engine_nums or {}) do
