...
if RequiredScript == "lib/managers/hudmanager" or RequiredScript == "lib/network/base/networkpeer" then
	...
	local function set_teammates_information(peer_id, outfit_string)
		...
		if not outfit then
			local peer = peer_id and managers.network and managers.network:session() and managers.network:session():peer(peer_id)
			outfit = peer and peer.blackmarket_outfit and peer:blackmarket_outfit()
		end
