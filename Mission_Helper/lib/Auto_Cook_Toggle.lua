...
elseif Global.game_settings and Global.game_settings.level_id == "pal" then -- for Counterfeit
	...
	local function check_make_counterfeit()
		DelayedCalls:Add("check_make_counterfeit", 2, function()
			if not global_autocook_toggle then
				return
			end
			for _, unit in pairs(managers.interaction._interactive_units) do
...
elseif Global.game_settings and (Global.game_settings.level_id == "roberts" or Global.game_settings.level_id == "nmh") then -- for GO Bank and No Mercy
	...
	local function check_answer_phone()
		DelayedCalls:Add("check_answer_phone", 2, function()
			if not global_autocook_toggle or not managers.groupai:state():whisper_mode() then
				return
			end
			local player_unit = managers.player and alive(managers.player:player_unit()) and managers.player:player_unit()
			for _, unit in pairs(managers.interaction and managers.interaction._interactive_units or {}) do
				...
				if player_unit and interaction then
					if interaction.tweak_data == "hospital_phone" then
						interaction:interact(player_unit)
						managers.mission._fading_debug_output:script().log("Answer Phone",  Color.yellow)
					elseif interaction.tweak_data == "bank_note" then
						interaction:interact(player_unit)
					end
				end
			end
