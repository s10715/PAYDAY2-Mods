...
local function secret_painting_puzzle()
	...
	local function kill_dentist()
		local run_now = true
		local dentist_unit = nil
		local function do_kill_dentist()
			DelayedCalls:Add("do_kill_dentist", run_now and 0.01 or 2, function()
				run_now = false
				if not dentist_unit then
					for _, unit in pairs(World:find_units_quick("all", managers.slot:get_mask("all")) or {}) do
						if alive(unit) and unit:name() == Idstring("units/pd2_dlc_uno/props/uno_prop_dentist/uno_prop_dentist") then
							dentist_unit = unit
							break
						end
					end
				end
				if alive(dentist_unit) and dentist_unit.num_bodies then
					for i = 0, dentist_unit:num_bodies() do
						local body = dentist_unit:body(i)
						if (body and body:enabled()) and (body:unit():id() ~= -1) and body:extension() and body:extension().damage and body:extension().damage.damage_melee then
							body:extension().damage:damage_melee(dentist_unit, nil, body:position(), nil, 10000)
							managers.network:session():send_to_peers_synched("sync_body_damage_melee", body, dentist_unit, nil, body:position(), nil, 10000)
						end
					end
					if dentist_unit.damage and dentist_unit:damage() and type(dentist_unit:damage()._damage) == 'number' and dentist_unit:damage()._damage > 0 then
						managers.mission._fading_debug_output:script().log('Dentist Killed',  Color.yellow)
						return true
					end
				end
				do_kill_dentist()
...
local function big_bank_crane_drop()
	if Global.level_data and Global.level_data.level_id == "big" and not managers.groupai:state():whisper_mode() then
		for _, unit in pairs(managers.interaction._interactive_units or {}) do
			...
			if interaction and interaction.can_interact and interaction:can_interact() and interaction.active and interaction:active() and (interaction.tweak_data == "crane_joystick_left" or interaction.tweak_data == "crane_joystick_release") then
			...
			if alive(unit) then
				...
				if timer_gui then
					if timer_gui._started and timer_gui._update_enabled and not timer_gui._done then
						...
						for i = 1, 4 do
							DelayedCalls:Add("anti_jam_" .. tostring(unit_idx) .. "_" .. tostring(i), 1 * i, function()
